...
> ⚠️ **歴史的資料（現状の実装とは異なる）** — 2026-08-20 追記  
> ここで構想した「Next.js + Cloudflare Workers/D1 の専用ツール」は**未実装**。X投稿は現在、AI社員「ツムギ」（`.claude/agents/tsumugi-sns.md`）が投稿案を作り、編集長が数値チェックして社長が承認・投稿する運用に置き換わった。`src/`（Astroブログ）には該当コードは無い。将来この構想を復活させる場合の参照用として残置。
