...
- **最新記事セクション:** `PostCard` コンポーネントを2列グリッドで最新2件表示（2026-08時点。当初は4件だったが、下のカテゴリタブと合わせて総露出を増やす形に変更）
- **カテゴリタブ（2026-08追加）:** 「すべて／お金／セキュリティ／AI・開発」の4タブ。JavaScriptを使わず、隠しラジオボタン＋CSSだけで切り替える（全記事はビルド時にHTML出力済みなので検索エンジンからも見える）。選ばれたタブの記事を最大6件、`PostCard` の2列グリッドで表示。「AI・開発」はタブ表示名のみで、記事の`category`データは既存の「開発」のまま変更していない
- **「このブログについて」（2026-08追加）:** ページ最下部に運営者のアバター・簡単な紹介・About へのリンクを表示（誰が書いているサイトかをトップでも示す信頼性対策）。狭い画面では縦積みに切り替わる
